import { Client, createClient } from "../pi-aws-client";
...
>(
  ...
  const { organizationId, apiOrigin, uploadStoreInitialValue } = plugin.options;

  let client: Client;
  try {
    client = createClient({ organizationId, apiOrigin });
  } catch (error) {
    console.error(error);
  }
